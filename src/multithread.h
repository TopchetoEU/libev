...
#include <assert.h>
	...
	static inline int ev_cond_timewait(ev_cond_t cond, ev_mutex_t mut, ev_time_t timeout) {
		ev_time_t curr;
		ev_monotime(&curr);
		...
		if (!SleepConditionVariableCS(cond, mut, ms)) return ETIMEDOUT;
		return 0;
	}
	...
	static inline void ev_cond_new(ev_cond_t cond) {
		...
		assert(pthread_condattr_setclock(attr, CLOCK_MONOTONIC) == 0);
	...
	static inline int ev_cond_timewait(ev_cond_t cond, ev_mutex_t mut, ev_time_t timeout) {
		return pthread_cond_timedwait(cond, mut, &(struct timespec) { .tv_sec = timeout.sec, .tv_nsec = timeout.nsec });
	}
#else
