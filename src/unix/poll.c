...
typedef struct ev_poll_req {
	...
	int fd;
	union {
		struct { char *data; size_t n; size_t offset; } rw;
	};
} *ev_poll_req_t;
...
static bool evi_poll_addfd(ev_async_t async, int fd, bool write, size_t *pi) {
	for (size_t i = 0; i < *pi; i++) {
		if (async->fds[i].fd == fd) {
			if (write) async->fds[i].events |= POLLOUT;
			else async->fds[i].events |= POLLIN;
			return true;
		}
	}
	...
	if (write) async->fds[*pi].events |= POLLOUT;
	else async->fds[*pi].events |= POLLIN;
	(*pi)++;
	return true;
}

static bool evi_async_poll(ev_async_t async, const ev_time_t *ptimeout, void **pticket, int *perr) {
	ev_poll_req_t timeout_udata = malloc(sizeof *timeout_udata);

	for (ev_poll_req_t it = async->req_head; it; it = it->next) {
		switch (it->type) {
			case EVI_POLL_PREAD:
			case EVI_POLL_READ:
				if (evi_poll_addfd(async, it))
		}
	}

	if (ptimeout) {
		#ifdef _GNU_SOURCE
			// GNU allows us to use ppoll, with the ability to pass a timespec as the timeout
		#else
			ev_time_t now;
			evs_monotime(&now);
			...
			if (diff.sec != 0 && diff.nsec > 1000000) {
				evs_sleep(*ptimeout);
			}
		#endif
	...
	while (true) {
		...
		ev_poll_req_t udata = (ev_poll_req_t)(size_t)cqe->user_data;
		if (!udata) {
...
static ev_code_t evi_async_read(ev_async_t async, void *ticket, ev_handle_t fd, char *buff, size_t *n) {
	ev_poll_req_t udata = evi_uring_mkudata(EVI_URING_RW, ticket);
...
static ev_code_t evi_async_file_open(ev_async_t async, void *ticket, ev_handle_t *pres, const char *path, ev_open_flags_t flags, int mode) {
	ev_poll_req_t udata = evi_uring_mkudata(EVI_URING_OPEN, ticket);
	if (!udata) return EV_ENOMEM;
	udata->phnd = pres;
...
static ev_code_t evi_async_stat(ev_async_t async, void *ticket, ev_handle_t fd, ev_stat_t *pres) {
	ev_poll_req_t udata = evi_uring_mkudata(EVI_URING_STAT, ticket);
	if (!udata) return EV_ENOMEM;
...
static ev_code_t evi_async_server_accept(ev_async_t async, void *ticket, ev_handle_t *pres, ev_addr_t *paddr, uint16_t *pport, ev_server_t server) {
	ev_poll_req_t udata = evi_uring_mkudata(EVI_URING_ACCEPT, ticket);
	if (!udata) return EV_ENOMEM;
...
static ev_code_t evi_async_socket_connect(ev_async_t async, void *ticket, ev_handle_t *pres, ev_proto_t proto, ev_addr_t addr, uint16_t port) {
	...
	ev_poll_req_t udata = evi_uring_mkudata(EVI_URING_CONNECT, ticket);
	if (!udata) return EV_ENOMEM;
...
static ev_code_t evi_async_proc_wait(ev_async_t async, void *ticket, ev_proc_t proc, int *psig, int *pcode) {
	ev_poll_req_t udata = evi_uring_mkudata(EVI_URING_WAIT, ticket);
	if (!udata) return EV_ENOMEM;
